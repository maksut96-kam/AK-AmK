...
# ============================================================
# 🟢 БЛОК 1: КОНФИГУРАЦИЯ И ДВИЖОК (DO NOT TOUCH)
# ============================================================
...
# ============================================================
# 🔵 БЛОК 2: СПРАВОЧНИКИ, ИКОНКИ И КОНСТАНТЫ
# ============================================================
...
Z_ICONS = {
    "Овен": "♈ Овен", "Телец": "♉ Телец", "Близнецы": "♊ Близн", 
    "Рак": "♋ Рак", "Лев": "♌ Лев", "Дева": "♍ Дева", 
    "Весы": "♎ Весы", "Скорпион": "♏Скорп", "Стрелец": "♐ Стрел", 
    "Козерог": "♑Козег", "Водолей": "♒ Водол", "Рыбы": "♓ Рыбы"
}
...
def get_dynamic_ayanamsa(t):
    ...
    return 23.856235 + (2.30142 * t_centuries) + (0.000139 * t_centuries**2)
...
def get_planet_data(t):
    ...
    lat, lon_m, dist = earth.at(t).observe(eph['moon']).ecliptic_latlon()
    ra_lon = (lon_m.degrees - current_ayan + 180) % 360 
    ra_deg = 30 - (ra_lon % 30) 
...
# ============================================================
# 🟣 БЛОК 4: ВИЗУАЛЬНЫЙ СТИЛЬ (CSS И ЧАСЫ)
# ============================================================
st.markdown("""
<style>
    .julia-title { text-align: center; margin-bottom: 25px; font-weight: 800; font-size: 3.2em; 
...
st.iframe("""
    <html><body style="background: linear-gradient(90deg, #050510, #0a0a20); margin:0; display:flex; flex-direction:column; align-items:center; justify-content:center; height:100px; font-family:sans-serif; border-radius:15px; border: 1px solid #1B263B;">
        <h2 id="clock" style="margin:0; color:#415A77; letter-spacing: 2px;">00:00:00</h2>
        <p style="margin:0; color:#778DA9; font-size: 0.8em; text-transform: uppercase;">Sochi Astro-Coordination Time (UTC+3)</p>
        <script>
            function updateClock() { let d = new Date(); let utc = d.getTime() + (d.getTimezoneOffset() * 60000); 
            let sochi = new Date(utc + (3600000 * 3)); document.getElementById('clock').innerHTML = sochi.toLocaleTimeString('ru-RU'); }
...
# ============================================================
# 🟠 БЛОК 5: ОСНОВНОЙ ИНТЕРФЕЙС (TABS)
# ============================================================
...
with tab1:
    ...
    with col_m2:
        st.metric("💎 Текущая АК", get_full_info(df.iloc[0]))
        st.metric("🥈 Текущая AmK", get_full_info(df.iloc[1]))

    st.divider()
    
    # Таблица данных
    st.subheader("📊 Аналитика Чара-карак")
    df_v = df.copy()
    ...
    # Использование stretch для корректного отображения ширины
...
with tab2:
    ...
    with c_p1:
        d_s = st.date_input("Дата начала", datetime.now(), key="start_d")
        t_s = st.time_input("Время начала", time(0,0), key="start_t")
    with c_p2:
        d_e = st.date_input("Дата конца", datetime.now() + timedelta(days=3), key="end_d")
        t_e = st.time_input("Время конца", time(23,59), key="end_t")

    if st.button('🚀 Сформировать бланк'):
        ...
        while curr_utc < end_utc:
            ...
            t_step = ts.utc(curr_utc.year, curr_utc.month, curr_utc.day, curr_utc.hour, curr_utc.minute)
            df_step, _, _ = get_planet_data(t_step)
            new_pair = f"{df_step.iloc[0]['Planet']}/{df_step.iloc[1]['Planet']}"
            if new_pair != last_pair:
                events.append({
                    "Время (Сочи)": (curr_utc + timedelta(hours=3)).strftime("%d.%m.%Y %H:%M"), 
