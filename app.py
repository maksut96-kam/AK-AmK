...
<style>
    ...
    @keyframes fly {
        ...
        20% { opacity: 1; }
        100% { transform: scale(18) translate(150px, 80px); opacity: 0; }
    }
...
with t1:
    ...
    <div class="moon-altar">
        <div style="display: flex; justify-content: space-between;">
            ...
            <div style="color: #778da9;">{"Растущая" if l['wax'] else "Убывающая"} • {int(l['illum'])}% освещенности</div></div>
        ...
        <div style="display: flex; justify-content: space-between; font-size: 0.85em;">
            <span>🌕 Полнолуние через: {int(l['to_full']//24)}д {int(l['to_full']%24)}ч</span>
            <span>🌑 Новолуние через: {int(l['to_new']//24)}д {int(l['to_new']%24)}ч</span>
        </div>
    ...
    # Ротации и Раху
    st.subheader("📋 Полный мониторинг карак")
    df_f = df_n.copy()
    df_f['Данные'] = df_f.apply(format_cell, axis=1)
    st.write(df_f[['Role', 'Planet', 'Deg', 'Данные']].to_html(escape=False, index=False), unsafe_allow_html=True)
    
    r_deg = df_n[df_n['Planet']=='Rahu'].iloc[0]['Deg']
    st.warning(f"🐲 **Монитор Раху:** {r_deg:.2f}° | Статус: {'⚠️ Опасная зона (переход)' if r_deg < 2 or r_deg > 28 else '✅ Стабилен'}")

with t2:
    st.subheader("⚙️ Генератор сетки ротаций")
    cx, cy = st.columns(2)
    with cx:
        ds = st.date_input("Дата начала", datetime.now())
        ts_i = st.time_input("Время начала", time(0, 0))
    with cy:
        de = st.date_input("Дата конца", datetime.now() + timedelta(days=2))
        te_i = st.time_input("Время конца", time(23, 59))
    ...
    if st.button("🚀 ЗАПУСТИТЬ РАСЧЕТ"):
        ...
        curr = start_u
        total_delta = (end_u - start_u).total_seconds()
        ...
        results.append({
            ...
            "💎 АК": format_cell(df_init.iloc[0]),
            "🥈 AmK": format_cell(df_init.iloc[1])
        })

        while curr < end_u:
            curr += timedelta(minutes=5)
            pb.progress(min(1.0, (curr - start_u).total_seconds() / total_delta))
            status_txt.text(f"Анализ: {curr.strftime('%d.%m %H:%M')}")
            ...
            if new_pair != last_pair:
                results.append({
                    ...
                    "День": RU_DAYS[(curr + timedelta(hours=3)).weekday()],
                    "Время": (curr + timedelta(hours=3)).strftime("%H:%M"),
                    "💎 АК": format_cell(df_eval.iloc[0]),
                    "🥈 AmK": format_cell(df_eval.iloc[1])
                })
                last_pair = new_pair
        
        pb.empty()
        status_txt.success("Расчет таблицы завершен!")
        
        if results:
            df_res = pd.DataFrame(results)
            
            # --- ФИНАЛЬНЫЙ СКРИПТ ПЕЧАТИ (BLOB) ---
            html_content = df_res.to_html(escape=False, index=False).replace('\n', '')
            
            print_code = f"""
            <script>
            function openPrint() {{
                const html = `<html><head><title>Astro Report</title>
                <style>
                ...
                <body><h2>Отчет по ротациям АК/AmK</h2>{html_content}</body></html>`;
            ...
            <button onclick="openPrint()" style="width:100%; padding:18px; background:#28a745; color:white; border:none; border-radius:12px; cursor:pointer; font-weight:bold; font-size:1.1em; margin-bottom:20px;">
                🖨️ ПОДГОТОВИТЬ ТАБЛИЦУ К ПЕЧАТИ
            </button>
            """
            components.html(print_code, height=80)
