...
<style>
    ...
    @keyframes fly { 0% { transform: translateX(-100px); opacity: 0; } 15% { opacity: 1; } 85% { opacity: 1; } 100% { transform: translateX(1600px); opacity: 0; } }
...
def find_rotations(start_dt):
    ...
    for i in range(1, 1500, 10):
        ...
        if f"{df_p.iloc[0]['Planet']}-{df_p.iloc[1]['Planet']}" != current_pair:
            events.append({"type": "Предыдущая", "dt": t_check_dt + timedelta(hours=3), "ak": df_p.iloc[0], "amk": df_p.iloc[1]}); break
    for i in range(1, 1500, 10):
        t_check_dt = start_dt + timedelta(minutes=i)
...
# ============================================================
st.markdown("""
<div class="header-box"><h1 class="main-title">JULIA ASSISTANT</h1><div class="sub-title">Astro coordination center</div></div>
<div class="space-banner">
    <div class="stars"></div>
    <div class="planet p1"></div><div class="planet p2"></div><div class="planet p3"></div><div class="planet p4"></div>
...
with t2:
    ...
    if st.button("🚀 ПОСТРОИТЬ ГРАФИК"):
        ...
        if results:
            ...
            st.markdown(f"""<script>function printDiv() {{ var win = window.open('', '_blank'); win.document.write('<html><head><title>Print</title><style>table{{border-collapse:collapse;width:100%;font-family:sans-serif;}}th,td{{border:1px solid #000;padding:10px;font-size:11px;}}b{{color:#000;}}span{{color:#333;}}</style></head><body>' + `{clean_html}` + '</body></html>'); win.document.close(); win.print(); }}</script><button onclick="printDiv()" style="width:100%; padding:20px; background:#28a745; color:white; border:none; border-radius:10px; cursor:pointer; font-weight:bold; font-size:1.2em; margin-bottom:20px;">🖨️ ПЕЧАТЬ ТАБЛИЦЫ</button>""", unsafe_allow_html=True)
